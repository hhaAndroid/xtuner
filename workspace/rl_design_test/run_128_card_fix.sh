...
export CONFIG_PATH='./workspace/rl_design_test/interns2-35ba3-base05-20260424a-rl-data260426rc1-56k-badword-mtp4-0528.py'
# export WORK_DIR='/mnt/shared-storage-user/duanyanhui/workspace/dev/xtuner-interns2-refactor/log-128card-0503'
export WORK_DIR="./work_dirs/qwen35/xtuner-interns2-refactor/64card-delivery-exp-0529-${current_time}"
...
if [ "$RAY_RANK" -eq 0 ]; then
  ...
  mkdir -p "${RAY_LOG_DIR}"
  # Use an absolute symlink target. A relative target under /tmp becomes a
  # dangling link and Ray's os.makedirs('/tmp/ray_log/', exist_ok=True) raises
  # FileExistsError before the head node starts.
