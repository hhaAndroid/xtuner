...
script=${2:-"/mnt/shared-storage-user/huanghaian/code/temp/xtuner/workspace/new_agent_dev/qwen35_rl_localagent.sh"}
...
export NUM_WORKERS=$((node_num * gpus))

export CLUSTERX_CFG_PATH=/mnt/shared-storage-user/huanghaian/clusterx.yaml
export PIP_INDEX_URL=$PIP_INDEX_URL
export PIP_TRUSTED_HOST=$PIP_TRUSTED_HOST
clusterx run -N $node_num --gpus-per-task $gpus --priority 9 --job-name $job_name --cpus-per-task $cpu --memory-per-task $mem --image $IMAGE_URL \
  sh $script 
