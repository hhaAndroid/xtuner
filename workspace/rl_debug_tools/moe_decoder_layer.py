...
class MoEDecoderLayer(nn.Module):
    ...
    def _forward(
        ...
        seq_ctx.TSaver.save(f'layer{self.layer_idx}.mlp.output_hidden_states', combined_hidden_states)
        hidden_states = self._post_moe_forward(
            combined_hidden_states=combined_hidden_states,
            residual=residual,
            shared_experts_out=shared_experts_out,
        )
        seq_ctx.TSaver.save(f'layer{self.layer_idx}.output_hidden_states', hidden_states)
    ...
    def _pre_moe_forward(
        ...
        hidden_states = seq_ctx.TSaver.replace_tensor(f'layer{self.layer_idx}.input_hidden_states', hidden_states)
        seq_ctx.TSaver.save(f'layer{self.layer_idx}.input_hidden_states', hidden_states)
        residual = hidden_states
        hidden_states = self.input_layernorm(hidden_states)
        hidden_states = seq_ctx.TSaver.replace_tensor(f'layer{self.layer_idx}.attn.input_hidden_states', hidden_states)
        seq_ctx.TSaver.save(f'layer{self.layer_idx}.attn.input_hidden_states', hidden_states)
        # Self Attention
        ...
        seq_ctx.TSaver.save(f'layer{self.layer_idx}.attn.output_hidden_states', hidden_states)
        ...
        hidden_states = seq_ctx.TSaver.replace_tensor(f'layer{self.layer_idx}.mlp.input_hidden_states', hidden_states)
        seq_ctx.TSaver.save(f'layer{self.layer_idx}.mlp.input_hidden_states', hidden_states)
        
        router_results: RouterResults = self.gate(hidden_states, rollout_routed_experts)
        ## router_results['logits'] = seq_ctx.TSaver.replace_tensor(f'layer{self.layer_idx}.mlp.gate.logits', router_results['logits']).float()
        ## router_results['topk_weights'] = seq_ctx.TSaver.replace_tensor(f'layer{self.layer_idx}.mlp.gate.topk_weights', router_results['topk_weights'])
        seq_ctx.TSaver.save(f'layer{self.layer_idx}.mlp.gate.logits', router_results["logits"].unsqueeze(0))
        seq_ctx.TSaver.save(f'layer{self.layer_idx}.mlp.gate.topk_ids', router_results["topk_ids"].unsqueeze(0))
        # seq_ctx.TSaver.save(f'layer{self.layer_idx}.mlp.gate.topk_weights', router_results["topk_weights_before_norm"].unsqueeze(0))
        router_results["topk_ids"] = router_results["topk_ids"].contiguous()
