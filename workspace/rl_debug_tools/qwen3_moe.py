...
from xtuner.v1.utils import ForkedPdb
...
from extra_helper import TensorSaver

if os.getenv('CUBLAS_WORKSPACE_CONFIG') == ":4096:8":
    torch.use_deterministic_algorithms(True)

WORKDIR = os.environ.get('res_lmdeploy_dir', None)
TSaver = TensorSaver(WORKDIR)
...
class Qwen3MoeSparseMoeBlock(nn.Module):
    ...
    def forward(
        ...
        TSaver.save(f'layer{self.layer_idx}.mlp.gate.logits', router_logits.unsqueeze(0))
        TSaver.save(f'layer{self.layer_idx}.mlp.gate.topk_ids', topk_ids.unsqueeze(0))
        TSaver.save(f'layer{self.layer_idx}.mlp.gate.topk_weights', topk_weights.unsqueeze(0))
        get_dist_manager().current_context().TSaver = TSaver
...
class Qwen3MoeDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        TSaver.save(f'layer{self.layer_idx}.input_hidden_states', hidden_states)
        ...
        # Self Attention
        TSaver.save(f'layer{self.layer_idx}.attn.input_hidden_states', hidden_states)
        ...
        TSaver.save(f'layer{self.layer_idx}.attn.output_hidden_states', hidden_states)
        hidden_states = residual + hidden_states

        # Fully Connected
        residual = hidden_states
        hidden_states = self.post_attention_layernorm(hidden_states)
        TSaver.save(f'layer{self.layer_idx}.mlp.input_hidden_states', hidden_states)
        hidden_states = self.mlp(hidden_states, all_routed_experts=all_routed_experts)
        TSaver.save(f'layer{self.layer_idx}.mlp.output_hidden_states', hidden_states)
        hidden_states = hidden_states + residual
        TSaver.save(f'layer{self.layer_idx}.output_hidden_states', hidden_states)
...
class Qwen3MoeForCausalLM(nn.Module, CudaGraphMixin):
    ...
    def forward(
        ...
        **kwargs,
        ...
        TSaver.step()
        TSaver.save('input_ids', input_ids)
        # router replay
        ...
        TSaver.save('last_hidden_states', hidden_states)
    ...
    def get_logits(self, hidden_states: torch.Tensor):
        ...
        TSaver.save('lm_head.input_hidden_states', hidden_states)
        logits =  self.lm_head(hidden_states.float())
        TSaver.save('logits', logits)
        return logits
