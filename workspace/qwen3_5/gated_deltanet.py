...
class GatedDeltaNet(nn.Module):
    ...
    def forward(
        ...
        print(self.conv1d.weight.shape, mixed_qkv.shape)

        weight = self.conv1d.weight.squeeze(1) # 8192, 4
        ...
        if seq_ctx.sequence_parallel_mesh and seq_ctx.sequence_parallel_mesh.size() > 1:
            ...
            weight = weight.chunk(seq_ctx.sequence_parallel_mesh.size(), dim=0)[sp_rank] # 4096,4
            bias = self.conv1d.bias
            if bias is not None:
        ...
        # TODO: remove the all_to_all
        if seq_ctx.sequence_parallel_mesh and seq_ctx.sequence_parallel_mesh.size() > 1:
                # gather the output of conv1d from all sequence parallel ranks
                mixed_qkv = ulysses_all_to_all(
                    mixed_qkv,
                    scatter_dim=2,
                    gather_dim=1,
