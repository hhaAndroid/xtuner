...
def forward_xtuenr(path, inputs):
    ...
    # TODO: pack ....
    input_ids = inputs.input_ids # 2,n
    attention_mask = inputs.attention_mask # 2,n

    input_ids_1 = input_ids[0:1,:] # 1,n
    input_ids_1 = input_ids_1[:, :attention_mask[0].sum()] # 1,seq_len
    labels_1 = input_ids_1.clone()
    labels_1[:,0] = -100

    input_ids_2 = input_ids[1:2,:] # 1,n
    input_ids_2 = input_ids_2[:, :attention_mask[1].sum()] # 1,seq_len
    labels_2 = input_ids_2.clone()
    labels_2[:,0] = -100

    input_ids = torch.cat([input_ids_1, input_ids_2], dim=-1)
    labels = torch.cat([labels_1, labels_2], dim=-1)

    # input_ids = input_ids_1
    # labels = labels_1

    num_tokens = attention_mask.sum(dim=1).tolist() # [seq_len1, seq_len2]
    num_tokens[-1] -= 1
    ...
    seq_ctx = SequenceContext(
        input_ids=shift_input_ids,  # type: ignore
...
if __name__ == '__main__':
    ...
    #         "role": "user",
    #         "content": [
    #             {"type": "text", "text": "你觉得北京如何？我觉得北京蛮好的，你咋看呢？"},
    #         ],
    #     }
    # ]
