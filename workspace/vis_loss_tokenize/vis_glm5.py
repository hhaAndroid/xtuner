...
def tokenize_fn_slowspeed(tokenizer, messages: List[Dict[str, str]], tools=None,  **kwargs):
    """
    ...
    for i, msg in enumerate(messages):
        if msg['role'] == 'assistant' and msg.get('loss', True):
            ...
            prompt_text = tokenizer.apply_chat_template(messages[:i], tokenize=False, add_generation_prompt=True, tools=tools if i==0 else None)
            # 2. 获取包含当前消息的完整“截断”文本
            ...
            # 提取真实内容
            m_text = tokenizer.apply_chat_template(temp_msgs, tokenize=False, tools=tools if i==0 else None)
            # 转换为 Token 序列
...
def show():
    ...
    jsonl_path = '/mnt/shared-storage-user/huanghaian/code/temp/xtuner/workspace/qwen3_5/tokenize_data_demo_q35.jsonl'
    all_data= []
    ...
    for j, data in enumerate(all_data):
        ...
        token_ids, labels = tokenize_fn_slowspeed(tokenizer, data['messages'], tools=data.get('tools'))
        end_time = time.time()
