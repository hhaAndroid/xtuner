...
import json
from xtuner.v1.datasets import OpenaiTokenizeFunction
...
def show():
    ...
    demo_data_path='/mnt/shared-storage-user/huanghaian/code/temp/xtuner/tests/resource/qwen35_tokenize_data.jsonl'

    tokenize_fn = OpenaiTokenizeFunction(tokenizer, chat_template="qwen3.5-vl")
    all_data = []
    with open(demo_data_path, 'r') as f:
        for line in f:
            all_data.append(json.loads(line))
    messages = all_data[6]
    messages['messages'][2]["content"] = "<think>\n\n</think>\n\n我需要先调用一些工具才能知道\n"
    ...
    # token_ids, labels = tokenize_fn_slowspeed(tokenizer, messages['messages'],tools=messages.get('tools'))  # 双重验证，确保两套 tokenize 结果一致
    ...
    # 判断 token_ids 中出现这种模式情况下，labels 是否正确地将这部分标记为负样本
    for i in range(len(token_ids) - len(_skip_seq_1) + 1):
        if token_ids[i : i + len(_skip_seq_1)] == _skip_seq_1:
            # 这部分应该是负样本
            assert all(l < 0 for l in labels[i : i + len(_skip_seq_1)]), "Error: Detected <think>...</think> pattern not marked as negative samples!"
    for i in range(len(token_ids) - len(_skip_seq_2) + 1):
        if token_ids[i : i + len(_skip_seq_2)] == _skip_seq_2:
            # 这部分应该是负样本
            assert all(l < 0 for l in labels[i : i + len(_skip_seq_2)]), "Error: Detected <think>... pattern not marked as negative samples!"
