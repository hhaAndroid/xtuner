...
# Collect one lightweight Ray/Linux memory debugging snapshot.
#
# Typical usage on the Ray head node:
#
#   WORK_DIR=/path/to/ray/work_dir \
#   OUT_ROOT=/path/to/output_dir \
#   SMAPS_TOP_N=30 \
#   RAY_STATE_LIST_LIMIT=100 \
#   bash workspace/job_sh/debug.sh
#
# Optional environment variables:
#   WORK_DIR                Directory to cd into before running Ray CLI.
#                           Defaults to the current experiment path used by
#                           the original investigation.
#   OUT_ROOT                Parent directory for mem_YYYYMMDD_HHMMSS snapshots.
#                           Defaults to WORK_DIR. Falls back to /tmp if it is
#                           not writable.
...
#
# Output files:
#   meta.txt                Snapshot metadata and effective options.
#   ray_status.txt          Ray cluster resource health.
#   memory.txt              Aggregate Ray object store stats.
#   summary_objects.txt     Ray ObjectRef summary grouped by callsite.
#   summary_tasks.txt       Ray task state summary.
#   summary_actors.txt      Ray actor state summary.
#   tasks.json              Detailed Ray task sample, capped by limit.
#   actors.json             Detailed Ray actor sample, capped by limit.
#   objects.json            Optional detailed Ray object sample.
#   top_rss.txt             Local node processes sorted by RSS.
#   smaps_pids.txt          PIDs selected for smaps_rollup.
#   smaps_<pid>.txt         Per-process smaps_rollup copies.

WORK_DIR=${WORK_DIR:-/mnt/shared-storage-user/huanghaian/code/xtuner/work_dirs_rl_6}
