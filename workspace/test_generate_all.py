...
AIOHTTP_TIMEOUT = 30
# ['http://10.102.250.64:39097', 'http://10.102.250.64:40121', 'http://10.102.250.64:41145', 'http://10.102.250.64:42169']
port = 24546
api_url=f"http://0.0.0.0:{port}/generate"
...
async def send_all(num):
    ...
    limits = httpx.Limits(max_connections=8192, max_keepalive_connections=10000)
    client = httpx.AsyncClient(limits=limits, timeout=3600)

    print(f"Start sending {num} requests")
    tasks = [send_req(client, payload, idx) for idx in range(num)]
    print(f"All {num} requests sent, waiting for responses...")
