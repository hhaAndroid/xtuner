...
class RolloutController:
    ...
    def _restart_failed_workers(self, worker: RolloutWorker, expected_url: str) -> bool:
        try:
            ...
            _, session_url = ray.get(worker.get_session_server_info.remote(), timeout=ROLLOUT_RAY_GET_TIMEOUT)
    ...
    def _broadcast_to_active_workers(self, method_name: str, *args, **kwargs):
        ...
        with self.worker_info_lock:
            active_actors = [info.actor for info in self.rank2info.values() if info.is_active]
        futures = [getattr(actor, method_name).remote(*args, **kwargs) for actor in active_actors]
