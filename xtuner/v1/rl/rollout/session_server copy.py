...
class SessionServer:
    ...
    async def on_request(self, req_body: dict) -> dict:
        ...
        if session_id is None:
            raise web.HTTPBadRequest(text="Missing required field: session_id")
        input_ids = req_body.get("prompt_ids") or req_body.get("input_ids")
    ...
    async def on_response(self, worker_resp: dict) -> dict:
        ...
        if False and raw_routed_expert is not None:
    ...
    async def _handle_request(self, request: web.Request) -> web.Response:
        try:
            ...
            get_logger().warning(
                f"SessionServer request rejected: remote={request.remote} method={request.method} "
                f"path={request.path_qs} status={exc.status} reason={exc.reason}"
            )
            raise 
        except BaseException:
            get_logger().exception(
                f"SessionServer request failed: remote={request.remote} method={request.method} path={request.path_qs}"
            )
            raise
    ...
    async def _handle_request_impl(self, request: web.Request) -> web.Response:
        ...
        def _clean_data(data: dict) -> bool:
            ...
            for c in data.get("choices", []):
                if self.stop_word and c.get("message") and isinstance(c["message"].get("content"), str):
                    if self.stop_word in c["message"]["content"]:
                        c["message"]["content"] = c["message"]["content"].replace(self.stop_word, "")
                        modified = True
                if self.stop_word and c.get("delta") and isinstance(c["delta"].get("content"), str):
