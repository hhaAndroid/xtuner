...
class RolloutWorker(SingleAcceleratorWorker):
    ...
    def _start_session_server(self) -> None:
        ...
        self.session_server_actor = (
            ...
            .remote(
                ...
                max_model_len_reserved_tokens=self._get_session_server_reserved_tokens(),
                normalize_sglang_sampling_params=self.config.rollout_backend == "sglang",
                drain_upstream_on_client_disconnect=self.config.rollout_backend != "sglang",
    ...
    def _get_session_server_reserved_tokens(self) -> int:
        extra_config = self.config.extra_rollout_config or {}
        algorithm = str(extra_config.get("sglang_speculative_algorithm") or "").lower()
        if not algorithm.startswith("eagle"):
            return 0

        def _int_config(name: str, default: int) -> int:
            value = extra_config.get(name, default)
            try:
                return int(value)
        ...
        return max(
            _int_config("sglang_speculative_eagle_topk", 1) * _int_config("sglang_speculative_num_steps", 1),
            _int_config("sglang_speculative_num_draft_tokens", 0),
        )
