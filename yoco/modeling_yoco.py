...
from .configuration_yoco import YOCOConfig
...
class InternLM2WindowFlashAttention2(nn.Module):
    """
...
class InternLM2SelfDecoderLayer(nn.Module):
    ...
    def __init__(self, config, layer_idx: int):
        ...
        self.attention = InternLM2WindowFlashAttention2(config=config, layer_idx=layer_idx)
...
class InternLM2CrossDecoderLayer(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.hidden_size = config.hidden_size
        self.attention = InternLM2CrossFlashAttention2(config=config)
        self.feed_forward = InternLM2MLP(config)
        self.attention_norm = InternLM2RMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        self.ffn_norm = InternLM2RMSNorm(config.hidden_size, eps=config.rms_norm_eps)
...
class InternLM2CrossDecoder(nn.Module):
    """InternLM2 Decoder Layer. This module is a single layer of the InternLM2 model."""
    ...
    def __init__(self, config, num_cross_decoder_layers):
        ...
        # global kv layer norm and projection
        self.num_key_value_heads = config.num_key_value_heads
        self.head_dim = self.hidden_size // config.num_attention_heads
        self.kv_layer_norm = InternLM2RMSNorm(config.hidden_size, eps=config.rms_norm_eps)
    ...
    def forward(
        ...
        key_states = key_states.view(bsz, seqlen, self.num_key_value_heads, self.head_dim).transpose(1, 2)
        value_states = value_states.view(bsz, seqlen, self.num_key_value_heads, self.head_dim).transpose(1, 2)
...
class YOCOInternLM2Model(YOCOInternLM2PreTrainedModel):
    """
    ...
    def __init__(self, config):
        ...
        num_self_decoder_layers = config.num_self_decoder_layers
        num_cross_decoder_layers = config.num_hidden_layers - num_self_decoder_layers

        self.self_layers = nn.ModuleList(
            [InternLM2SelfDecoderLayer(config, layer_idx) for layer_idx in range(num_self_decoder_layers)]
        )
        self.cross_layers = InternLM2CrossDecoder(config, num_cross_decoder_layers)
    ...
    def forward(
        ...
        for decoder_layer in self.self_layers:
        ...
        hidden_states = self.cross_layers(hidden_states,
                                          attention_mask=causal_mask,
                                          position_ids=position_ids,
                                          past_key_value=past_key_values,
                                          output_attentions=output_attentions,
...
class YOCOInternLM2ForCausalLM(YOCOInternLM2PreTrainedModel):
    ...
    def __init__(self, config):
        super().__init__(config)
        self.model = YOCOInternLM2Model(config)
        self.vocab_size = config.vocab_size
