...
from xtuner._lite.datasets.llava import (NewLlavaCollator, LlavaDatasetForNonPack, NewLlavaTokenizeFunction)
from xtuner._lite.datasets.load_new import DATASET_CLS_MAP, load_datasets, SoftPackDataset
...
def llava_sft(args):
    ...
    for _, info in dset_infos.items():
        ...
        tokenize_fn = NewLlavaTokenizeFunction(tokenizer, chat_template,
    ...
    if args.dset_pack_level:
        ...
        # 不需要 packing 情况下不需要缓存,直接重写即可
        for name, _ in DATASET_CLS_MAP.items():
    ...
    collator = NewLlavaCollator(pack_batch=pack_batch)
